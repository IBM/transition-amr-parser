...
from neural_parser.fairseq_ext.data import indexed_dataset
from neural_parser.fairseq_ext import options
from neural_parser.fairseq_ext.extract_bart.binarize_encodings import make_bart_encodings
        ...
        if args.joined_dictionary:
            ...
            if target:
                if args.tgtdict:
                    ...
                    tgt_dict = task.build_dictionary_bart_extend(
                        node_freq_min=args.node_freq_min,
                        node_file_path=train_path(args.target_lang_vocab_nodes),
                        others_file_path=train_path(args.target_lang_vocab_others)
                        )
                    # copy the files needed to build the dictionary
                    shutil.copyfile(train_path(args.target_lang_vocab_nodes),
                                    os.path.join(args.destdir, args.target_lang_vocab_nodes))
                    shutil.copyfile(train_path(args.target_lang_vocab_others),
                                    os.path.join(args.destdir, args.target_lang_vocab_others))
            else:
                tgt_dict = None
