...
from neural_parser.fairseq_ext.data.data_utils import (
    collate_embeddings,
    collate_wp_idx,
    # collate_target_masks,
    # collate_masks
)
from neural_parser.fairseq_ext.data import data_utils
...
#     # DEBUG: Inline RoBERTa
    ...
    def merge_tgt_actedge_indexes(key, max_tgt_len):
        # sort, and transform index to batch_size * max_tgt_len index, and concat
        # default right padding: left_pad_target should be False
        merged = [samples[i][key] + j * max_tgt_len for i, j in zip(sort_order, range(len(sort_order)))]
        merged = torch.cat(merged, dim=0)
        return merged

    def merge_tgt_actedge_node_indexes(key):
        # sort, keep the index, and concat
        # default right padding: left_pad_target should be False
        merged = [samples[i][key] for i in sort_order]
        merged = torch.cat(merged, dim=0)
        return merged
    ...
    if collate_tgt_states:
        ...
        tgt_actedge_1stnode_masks = merge('tgt_actedge_1stnode_masks', left_pad=left_pad_target, pad_idx=0)
        tgt_actedge_1stnode_masks = tgt_actedge_1stnode_masks.index_select(0, sort_order)
        tgt_actedge_indexes = merge_tgt_actedge_indexes('tgt_actedge_indexes', target.size(1))
        tgt_actedge_cur_node_indexes = merge_tgt_actedge_node_indexes('tgt_actedge_cur_node_indexes')
        tgt_actedge_cur_1stnode_indexes = merge_tgt_actedge_node_indexes('tgt_actedge_cur_1stnode_indexes')
        tgt_actedge_pre_node_indexes = merge_tgt_actedge_node_indexes('tgt_actedge_pre_node_indexes')
        tgt_actedge_directions = merge_tgt_actedge_node_indexes('tgt_actedge_directions')
        # graph structure for connecting with all previous nodes (tied with tgt input)
        tgt_actedge_allpre_indexes = merge_tgt_actedge_indexes('tgt_actedge_allpre_indexes', target.size(1))
        tgt_actedge_allpre_pre_node_indexes = merge_tgt_actedge_node_indexes('tgt_actedge_allpre_pre_node_indexes')
        tgt_actedge_allpre_directions = merge_tgt_actedge_node_indexes('tgt_actedge_allpre_directions')
    else:
        ...
        tgt_actedge_1stnode_masks = None
        tgt_actedge_indexes = None
        tgt_actedge_cur_node_indexes = None
        tgt_actedge_cur_1stnode_indexes = None
        tgt_actedge_pre_node_indexes = None
        tgt_actedge_directions = None
        # graph structure for connecting with all previous nodes
        tgt_actedge_allpre_indexes = None
        tgt_actedge_allpre_pre_node_indexes = None
        tgt_actedge_allpre_directions = None
    ...
    batch = {
        ...
        'net_input': {
            ...
            'tgt_actedge_1stnode_masks': tgt_actedge_1stnode_masks,
            'tgt_actedge_indexes': tgt_actedge_indexes,
            'tgt_actedge_cur_node_indexes': tgt_actedge_cur_node_indexes,
            'tgt_actedge_cur_1stnode_indexes': tgt_actedge_cur_1stnode_indexes,
            'tgt_actedge_pre_node_indexes': tgt_actedge_pre_node_indexes,
            'tgt_actedge_directions': tgt_actedge_directions,
            # graph structure for connecting with all previous nodes
            'tgt_actedge_allpre_indexes': tgt_actedge_allpre_indexes,
            'tgt_actedge_allpre_pre_node_indexes': tgt_actedge_allpre_pre_node_indexes,
            'tgt_actedge_allpre_directions': tgt_actedge_allpre_directions
        },
        'target': target,
        'tgt_pos': tgt_pos
    }
...
class AMRActionPointerGraphMPDataset(FairseqDataset):
    ...
    def __init__(self, *,
                 ...
                 tgt_src_cursors=None,
                 tgt_actedge_masks=None,
                 tgt_actedge_1stnode_masks=None,    # the first node positions
                 tgt_actedge_indexes=None,
                 tgt_actedge_cur_node_indexes=None,
                 tgt_actedge_cur_1stnode_indexes=None,
                 tgt_actedge_pre_node_indexes=None,
        ...
        self.tgt_actedge_1stnode_masks = tgt_actedge_1stnode_masks
        self.tgt_actedge_indexes = tgt_actedge_indexes
        self.tgt_actedge_cur_node_indexes = tgt_actedge_cur_node_indexes
        self.tgt_actedge_cur_1stnode_indexes = tgt_actedge_cur_1stnode_indexes
        self.tgt_actedge_pre_node_indexes = tgt_actedge_pre_node_indexes
        self.tgt_actedge_directions = tgt_actedge_directions

        self.tgt_actedge_allpre_indexes = tgt_actedge_allpre_indexes
        self.tgt_actedge_allpre_pre_node_indexes = tgt_actedge_allpre_pre_node_indexes
        self.tgt_actedge_allpre_directions = tgt_actedge_allpre_directions
        ...
        if self.collate_tgt_states:
            ...
            assert self.tgt_actedge_1stnode_masks is not None
            assert self.tgt_actedge_indexes is not None
            assert self.tgt_actedge_cur_node_indexes is not None
            assert self.tgt_actedge_cur_1stnode_indexes is not None
            assert self.tgt_actedge_pre_node_indexes is not None
            assert self.tgt_actedge_directions is not None
            # graph structure for all previous nodes
            assert self.tgt_actedge_allpre_indexes is not None
            assert self.tgt_actedge_allpre_pre_node_indexes is not None
            assert self.tgt_actedge_allpre_directions is not None
    ...
    def __getitem__(self, index):
        ...
        tgt_actedge_1stnode_masks_item = self.tgt_actedge_1stnode_masks[index] \
            if self.tgt_actedge_1stnode_masks is not None else None
        tgt_actedge_indexes_item = self.tgt_actedge_indexes[index] \
            if self.tgt_actedge_indexes is not None else None
        tgt_actedge_cur_node_indexes_item = self.tgt_actedge_cur_node_indexes[index] \
            if self.tgt_actedge_cur_node_indexes is not None else None
        tgt_actedge_cur_1stnode_indexes_item = self.tgt_actedge_cur_1stnode_indexes[index] \
            if self.tgt_actedge_cur_1stnode_indexes is not None else None
        tgt_actedge_pre_node_indexes_item = self.tgt_actedge_pre_node_indexes[index] \
            if self.tgt_actedge_pre_node_indexes is not None else None
        tgt_actedge_directions_item = self.tgt_actedge_directions[index] \
            if self.tgt_actedge_directions is not None else None
        # graph structure for connecting the current node with all previous nodes
        tgt_actedge_allpre_indexes_item = self.tgt_actedge_allpre_indexes[index] \
            if self.tgt_actedge_allpre_indexes is not None else None
        tgt_actedge_allpre_pre_node_indexes_item = self.tgt_actedge_allpre_pre_node_indexes[index] \
            if self.tgt_actedge_allpre_pre_node_indexes is not None else None
        tgt_actedge_allpre_directions_item = self.tgt_actedge_allpre_directions[index] \
            if self.tgt_actedge_allpre_directions is not None else None

        # ========== tgt graph structure information should be tied with the tgt input ==========
        # a) the input vectors should be shifted to the right (we do it here instead of doing it in the batching)
        tgt_in_item = tgt_in_item.clone()
        tgt_actedge_masks_item = tgt_actedge_masks_item.clone()
        tgt_actedge_1stnode_masks_item = tgt_actedge_1stnode_masks_item.clone()  # 1st node mask on the tgt input side
        ...
        for tgt_in_x in (tgt_in_item,
                         tgt_actedge_masks_item,
                         tgt_actedge_1stnode_masks_item):
            tgt_in_x[1:] = tgt_in_x[:-1].clone()

        # b) for the </s> position at the beginning
        tgt_in_item[0] = self.tgt_dict.eos()
        tgt_actedge_masks_item[0] = 0
        tgt_actedge_1stnode_masks_item[0] = 0

        # c) the values referring to input action positions should be shifted 1 to the right
        tgt_actedge_indexes_item = tgt_actedge_indexes_item.clone() + 1
        tgt_actedge_cur_node_indexes_item = tgt_actedge_cur_node_indexes_item.clone() + 1
        tgt_actedge_cur_1stnode_indexes_item = tgt_actedge_cur_1stnode_indexes_item.clone() + 1
        tgt_actedge_pre_node_indexes_item = tgt_actedge_pre_node_indexes_item.clone() + 1
        tgt_actedge_allpre_indexes_item = tgt_actedge_allpre_indexes_item.clone() + 1
        tgt_actedge_allpre_pre_node_indexes_item = tgt_actedge_allpre_pre_node_indexes_item.clone() + 1
        ...
        if self.append_eos_to_target:
            ...
            # all of the target side data are with the CLOSE action (</s> in the vocab)
            tgt_item = tgt_item[:-1]
            ...
            tgt_actedge_1stnode_masks_item = tgt_actedge_1stnode_masks_item[:-1] \
                if tgt_actedge_1stnode_masks_item is not None else None
        ...
        return {
            ...
            'tgt_actedge_1stnode_masks': tgt_actedge_1stnode_masks_item,
            'tgt_actedge_indexes': tgt_actedge_indexes_item,
            'tgt_actedge_cur_node_indexes': tgt_actedge_cur_node_indexes_item,
            'tgt_actedge_cur_1stnode_indexes': tgt_actedge_cur_1stnode_indexes_item,
            'tgt_actedge_pre_node_indexes': tgt_actedge_pre_node_indexes_item,
            'tgt_actedge_directions': tgt_actedge_directions_item,
            # graph structure for connecting with all previous nodes (tied with the tgt input side)
            'tgt_actedge_allpre_indexes': tgt_actedge_allpre_indexes_item,
            'tgt_actedge_allpre_pre_node_indexes': tgt_actedge_allpre_pre_node_indexes_item,
            'tgt_actedge_allpre_directions': tgt_actedge_allpre_directions_item
        }
