...
from neural_parser.fairseq_ext.utils_import import import_user_module
from neural_parser.fairseq_ext import options
from neural_parser.fairseq_ext.utils import (post_process_action_pointer_prediction,
                               clean_pointer_arcs,
                               post_process_action_pointer_prediction_bartsv)
                               
from neural_parser.transition_amr_parser.force_overlap_actions import make_forced_overlap
...
def main(args):
    ...
    for (i,gen_subset) in enumerate(args.gen_subset.split(",")):

        print("decoding subset "+gen_subset)
        
        if i > 0:
            make_forced_overlap(args.results_path+"_"+str(i-1)+".actions",
                args.data+"/"+gen_subset+".en-actions.force_actions",
                args.data+"/"+gen_subset.split("_")[0]+".windows",
                i, args.data+"/"+gen_subset+".en-actions.force_actions")
                
    
        task.load_dataset(gen_subset, state_machine=False)
        ...
        examples = Examples(args.path, args.results_path+"_"+str(i), gen_subset, args.nbest,args.avoid_indices)
            ...
            for sample in t:
                ...
                for i, sample_id in enumerate(sample['id'].tolist()):
                    ...
                    if align_dict is not None:
                        src_str = task.dataset(gen_subset).src.get_original_text(sample_id)
                        target_str = task.dataset(gen_subset).tgt.get_original_text(sample_id)
                    else:
        ...
        if has_target:
            print('| Generate {} with beam={}: {}'.format(gen_subset, args.beam, scorer.result_string()))
            
        # change force actions for the next window
