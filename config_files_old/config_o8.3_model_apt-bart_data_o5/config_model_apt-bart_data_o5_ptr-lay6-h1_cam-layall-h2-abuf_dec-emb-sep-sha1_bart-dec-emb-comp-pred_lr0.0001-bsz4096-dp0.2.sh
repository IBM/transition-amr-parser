...
##### load data config
config_data=config_files/config_data/config_data_o8.3_bart-base.sh
...
bart_emb_init_composition=0
bart_emb_composition_pred=1
...
# PRED actions compositional embeddings
if [[ $bart_emb_composition_pred == 1 ]]; then
    dec_emb_comp_pred_tag="_bart-dec-emb-comp-pred"
else
    dec_emb_comp_pred_tag=""
fi

# combine different model configuration tags to the name
expdir=${expdir}${ptr_tag}${cam_tag}${tis_tag}${dec_emb_tag}${dec_emb_in_tag}${dec_emb_init_tag}${dec_emb_comp_pred_tag}${init_tag}${enc_fix_tag}${emb_fix_tag}
