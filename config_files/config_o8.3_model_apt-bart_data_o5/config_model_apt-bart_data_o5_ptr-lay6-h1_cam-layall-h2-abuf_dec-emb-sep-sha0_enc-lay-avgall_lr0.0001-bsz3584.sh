...
if [[ $src_avg_layers != "" ]]; then
    if [[ $src_avg_layers == "1 2 3 4 5 6" ]]; then
        enc_lay="all"
    else
        enc_lay=""
        for n in $src_avg_layers; do
            [[ $n < 1 || $n > 6 ]] && echo "Invalid 'src_avg_layers' input: $src_avg_layers" && exit 1
            enc_lay=$cam_lay$n
        done
    fi
...
expdir=${expdir}${ptr_tag}${cam_tag}${tis_tag}${dec_emb_tag}${dec_emb_in_tag}${dec_emb_init_tag}${init_tag}${enc_fix_tag}${emb_fix_tag}${enc_lay_tag}
