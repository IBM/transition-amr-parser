...
# # loss_coef
# if [[ $loss_coef != 1 ]]; then
#     loss_tag="-lmd${loss_coef}"
# else
#     loss_tag=""
# fi
# weight decay
if [[ $weight_decay != 0 ]]; then
    wd_tag="-wd${weight_decay}"
else
    wd_tag=""
fi
...
MODEL_FOLDER=$ROOTDIR/$expdir/models_ep${max_epoch}_seed${seed}_fix-lr${lr}-mt${max_tokens}-wm${warmup}-dp${dropout}${wd_tag}
