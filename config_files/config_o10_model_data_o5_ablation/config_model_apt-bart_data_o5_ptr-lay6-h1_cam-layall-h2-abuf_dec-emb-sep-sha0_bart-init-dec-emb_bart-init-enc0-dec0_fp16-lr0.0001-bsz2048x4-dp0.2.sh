...
if [[ $initialize_with_bart == 0 ]]; then
    ...
    # fi
    if [[ $initialize_with_bart_enc == 0 ]] && [[ $initialize_with_bart_dec == 1 ]]; then
        init_tag=_bart-init-enc0
    fi
    if [[ $initialize_with_bart_enc == 1 ]] && [[ $initialize_with_bart_dec == 0 ]]; then
        init_tag=_bart-init-dec0
    fi
    if [[ $initialize_with_bart_enc == 1 ]] && [[ $initialize_with_bart_dec == 1 ]]; then
        init_tag=""
    fi
    if [[ $initialize_with_bart_enc == 0 ]] && [[ $initialize_with_bart_dec == 0 ]]; then
        init_tag=_bart-init-enc0-dec0
    fi
fi
