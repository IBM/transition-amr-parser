...
##### load data config
config_data=config_files/config_data/config_data_amr1_graphmp-swaparc-ptrlast_o8.3_roberta-large-top24.sh
...
if [[ $tgt_graph_layers == "0 1 2" ]]; then
    grh_lay="all"
else
    grh_lay=""
    for n in $tgt_graph_layers; do
        [[ $n < 0 || $n > 2 ]] && echo "Invalid 'tgt_graph_layers' input: $tgt_graph_layers" && exit 1
        grh_lay=$grh_lay$(( $n + 1 ))
    done
fi
