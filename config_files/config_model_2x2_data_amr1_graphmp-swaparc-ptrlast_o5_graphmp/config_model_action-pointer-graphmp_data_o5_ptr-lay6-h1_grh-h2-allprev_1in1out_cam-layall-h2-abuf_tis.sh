...
##### load data config
config_data=config_files/config_data/config_data_amr1_graphmp-swaparc-ptrlast_o5_roberta-large-top24.sh
...
lay_cfg="_2x2"
arch=transformer_tgt_pointer_graphmp$lay_cfg
tgt_graph_layers="0"
tgt_graph_heads=2
tgt_graph_mask="allprev_1in1out"

pointer_dist_decoder_selfattn_layers="1"
pointer_dist_decoder_selfattn_heads=1
pointer_dist_decoder_selfattn_avg=0
pointer_dist_decoder_selfattn_infer=1

apply_tgt_src_align=1
tgt_src_align_layers="0 1"
...
if [[ $pointer_dist_decoder_selfattn_layers == "0 1" ]]; then
    lay="all"
else
    lay=""
    for n in $pointer_dist_decoder_selfattn_layers; do
        [[ $n < 0 || $n > 1 ]] && echo "Invalid 'pointer_dist_decoder_selfattn_layers' input: $pointer_dist_decoder_selfattn_layers" && exit 1
        lay=$lay$(( $n + 1 ))
    done
fi

if [[ $tgt_graph_layers == "0 1" ]]; then
    grh_lay="all"
else
    grh_lay=""
    for n in $tgt_graph_layers; do
        [[ $n < 0 || $n > 1 ]] && echo "Invalid 'tgt_graph_layers' input: $tgt_graph_layers" && exit 1
        grh_lay=$grh_lay$(( $n + 1 ))
    done
fi

if [[ $tgt_src_align_layers == "0 1" ]]; then
    cam_lay="all"
else
    cam_lay=""
    for n in $tgt_src_align_layers; do
        [[ $n < 0 || $n > 1 ]] && echo "Invalid 'tgt_src_align_layers' input: $tgt_src_align_layers" && exit 1
        cam_lay=$cam_lay$(( $n + 1 ))
    done
fi
...
expdir=exp_${data_tag}${lay_cfg}_act-pos-grh_vmask${tgt_vocab_masks}_shiftpos${shift_pointer_value}
