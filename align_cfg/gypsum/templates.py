...
python -u align_cfg/main.py --cuda --cache-dir $CACHE --vocab-text ./$CACHE/vocab.text.txt --vocab-amr ./$CACHE/vocab.amr.txt  --trn-amr ./$CACHE/train.txt.no_wiki --val-amr ./$CACHE/dev.txt.no_wiki --tst-amr ./$CACHE/test.txt.no_wiki --max-length 100 --log-dir $LOG2 --max-epoch 400  --batch-size 32 --accum-steps 4 --verbose  {flags} {model_cfg}
"""
# train
...
python align_cfg/run_eval.py --pred ./$LOG2/lstm_summer_2021.train.amr --gold ./$CACHE/lstm_summer_2021.train.amr
"""
...
python -u align_cfg/main.py --cuda --cache-dir $CACHE --vocab-text ./$CACHE/vocab.text.txt --vocab-amr ./$CACHE/vocab.amr.txt  --trn-amr ./$CACHE/dev.txt.no_wiki --val-amr ./$CACHE/dev.txt.no_wiki --tst-amr ./$CACHE/test.txt.no_wiki --max-length -1 --log-dir $LOG2 --max-epoch 400 --batch-size 32 --accum-steps 4 --verbose --skip-validation --load {load} --write-single --single-input ./$CACHE/dev.txt.no_wiki --single-output ./$LOG2/dev.aligned.txt {flags} {model_cfg}
...
python -u align_cfg/align_utils.py write_argmax --ibm-format --in-amr $CACHE/train.dummy_align.txt --in-amr-align-dist ./$LOG2/train.txt.align_dist.npy --out-amr-aligned ./$LOG2/train.txt

python -u align_cfg/align_utils.py verify_corpus_id --ibm-format --in-amr ./$LOG2/train.txt --corpus-id ./$LOG2/train.txt.align_dist.npy.corpus_hash
