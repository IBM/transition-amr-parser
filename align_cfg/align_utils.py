...
def main(args):
    if args.mode == 'write_argmax':
        ...
        """
        corpus = read_amr2(args.in_amr, ibm_format=args.ibm_format, tokenize=False)
        ...
        """
        Verify corpus matches already saved corpus id. Corpus ID is a hash generated by tokens and node ids.
        """
        corpus = read_amr2(args.in_amr, ibm_format=args.ibm_format, tokenize=False)
        corpus_id = hash_corpus(corpus)
...
if __name__ == '__main__':
    ...
    parser.add_argument('mode', choices=('write_argmax', 'compare', 'verify_corpus_id'), help="See main() for mode descriptions.")
    ...
    parser.add_argument('--ibm-format', action='store_true')
