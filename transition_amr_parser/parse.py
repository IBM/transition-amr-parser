...
def argument_parsing():
    ...
    parser.add_argument(
        ...
        type=str,
        default='en_XX',
        help='source language id from mBART pretraining to be appended to the source sentence'
    ),
    parser.add_argument(
        "--tgttag",
        type=str,
...
class AMRParser:
    ...
    def convert_sentences_to_data(self, sentences, batch_size, roberta_batch_size):
        ...
        for index, sentence in enumerate(sentences):
            ...
            ids = torch.cat((ids,a),0)
            wordpieces_roberta, word2piece_scattered_indices =\
                roberta_features[index]

            b = torch.IntTensor(1)
            b[0] = self.bart_dict.index("[{}]".format(self.srctag))
            wordpieces_roberta = torch.cat((wordpieces_roberta, b),0)
