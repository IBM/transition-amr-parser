...
class AMROracle():
    ...
    def get_actions(self, machine):
        ...
        for nid in self.align_by_token_pos[machine.cursor]:
            ...
            if normalize(machine.tokens[machine.cursor]) == target_node:
                # COPY
                return [('COPY', nid)], [1.0]
            else:
                # Generate
        ...
        if machine.cursor < len(machine.tokens):
            return ['SHIFT'], [1.0]
...
class AMRStateMachine():
    ...
    def get_valid_actions(self):
        ...
        valid_base_actions = ['COPY']
        if len(self.tokens) - self.cursor > 1:
            valid_base_actions.append('SHIFT')
        if len(self.node_stack) > 0:
            valid_base_actions.append('ROOT')
            if self.reduce_nodes:
                # TODO: Forbid disconnected nodes
        ...
        if self.root is not None and self.cursor == len(self.tokens) - 1:
    ...
    def update(self, action):
        ...
        # Node generation
            ...
            self.nodes[node_id] = normalize(self.tokens[self.cursor])
            self.node_stack.append(node_id)
            self.alignments[node_id].append(self.cursor)
...
def oracle(args):
    ...
    for amr in tqdm(amrs, desc='Oracle'):
