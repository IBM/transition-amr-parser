...
class AMRStateMachine:
    ...
    def __init__(self, tokens=None, tokseq_len=None, verbose=False, add_unaligned=0,
        ...
        # for multiple nodes from a single token: we need to further decouple node ids and token ids
        self._current_node_id = self.tokid_to_nodeid[self.tok_cursor]
    ...
    def apply_action(self, action, *, node2goldnode=None, goldnode=None):
        ...
        # the flowing 3 actions are for node generation
            ...
            self.PRED(properties[0], node2goldnode=node2goldnode, goldnode=goldnode)
        elif action_label == 'COPY_LEMMA':
            self.COPY_LEMMA(node2goldnode=node2goldnode, goldnode=goldnode)
        elif action_label in ['COPY_SENSE01']:
            self.COPY_SENSE01(node2goldnode=node2goldnode, goldnode=goldnode)
        # for multiple alignments and other cases
        elif action_label.startswith('DEPENDENT'):
            self.DEPENDENT(*properties, node2goldnode=node2goldnode, goldnode=goldnode)
        elif action_label in ['ADDNODE', 'ENTITY']:    # TODO 'ADDNODE' currently not used
            # preprocessing
            self.ENTITY(",".join(properties), node2goldnode=node2goldnode, goldnode=goldnode)
    ...
    def PRED(self, node_label, *, node2goldnode=None, goldnode=None):
        """PRED : assign a propbank label"""
        node_id = self.current_node_id

        # for multiple PRED out of one token
        # check if the current node_id has been used already --> use a new node id
        if node_id in self.is_confirmed:
            node_id = self.new_id
            self.new_id += 1
            self._current_node_id = node_id
            self.nodeid_to_tokid[node_id] = self.tok_cursor
        ...
        if node2goldnode is not None:
            node2goldnode[node_id] = goldnode

        return

    # TODO currently multiple nodes are only managed by PRED and ENTITY actions; change COPY actions if needed later
    def COPY_LEMMA(self, *, node2goldnode=None, goldnode=None):
    ...
    def COPY_SENSE01(self, *, node2goldnode=None, goldnode=None):
    ...
    def ENTITY(self, entity_type, *, node2goldnode=None, goldnode=None):
        ...
        # for multiple ENTITY PRED out of one token
        # check if the current node_id has been used already --> use a new node id
        if head in self.is_confirmed:
            head = self.new_id
            self.new_id += 1
            self._current_node_id = head
            self.nodeid_to_tokid[head] = self.tok_cursor
        ...
        self.entity_tokens[head] = surface_tokens
        ...
        # for oracle construction with gold AMR: build node id to gold node id mapping
        if node2goldnode is not None:
            node2goldnode[head] = goldnode    # here 'goldnode' could be a list

        return

    def DEPENDENT(self, node_label, edge_label, node_id=None, *, node2goldnode=None, goldnode=None):
