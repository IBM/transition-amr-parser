...
results_re = re.compile(r'(.+)_checkpoint_(wiki\.smatch.+?)\.pt\.(.+)')  # +? for non-greedy matching; only match the first "\."
# smatch_re = re.compile(r'valid_checkpoint([0-9]+)\.smatch')
# smatch_re_wiki = re.compile(r'valid_heckpoint([0-9]+)\.smatch')

# model names to consider
models = ['wiki.smatch_best1', 'wiki.smatch_top3-avg', 'wiki.smatch_top5-avg', 'wiki.smatch_top10-avg']
...
def get_scores_from_beam_dir(beam_dir, data_sets, models, score_name):
    ...
    for dfile in os.listdir(beam_dir):
        ...
        #print("data_set: ", data_set, " model_name: ", model_name, " sname: ", sname)
