...
if [ -f $ALIGNED_FOLDER/.done.preprocess ]; then
    ...
    # TODO(Ramon): Any tokenization, de-wikification, etc. should go here.
    ...
    python align_cfg/tokenize_amr.py \
        --in-amr ./DATA/AMR2.0/corpora/test.txt \
...
if [ -f $ALIGNED_FOLDER/.done.cache_data ]; then
    ...
    python align_cfg/vocab.py \
        --in-amrs $ALIGNED_FOLDER/train.unaligned.txt $ALIGNED_FOLDER/dev.unaligned.txt $ALIGNED_FOLDER/test.unaligned.txt \
...
if [ -f $ALIGNED_FOLDER/.done.extract ]; then
    ...
    touch $ALIGNED_FOLDER/.done.extract
